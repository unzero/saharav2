...
public class Conexion {
	...
	/* PUBLIC METHODS */
	private Conexion(Configuracion cn) throws IOException {
	...
	private void open(String path) throws IOException {
		LinkedList<String> rawData = new FileReader("./bin/resources/" + path)
				.getContent();
		for (String el : rawData) {
			Kanji newKanji = new Kanji(el);
			kanji_list.add(newKanji);
			for (String tmp : newKanji.get_lectures()) {
				if (all_lectures.indexOf(tmp) == -1)
					all_lectures.add(tmp);
			}
		}
	}
	...
	public static Conexion getInstance() throws IOException{
		if (instance == null) {
			instance = new Conexion(Configuracion.getInstance());
		}
		return instance;
	}
